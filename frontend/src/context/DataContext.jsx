...
import { trackingEvents as initialTrackingEvents, hubs as initialHubs, vehicles as initialVehicles, auditLogs as initialAuditLogs, cities, users, } from "@/data/mockData";
...
function mapApiParcel(row) {
    const mapped = {
        ...
        originCityId: row.pickup_city_id != null ? String(row.pickup_city_id) : "",
        destinationCityId: row.delivery_city_id != null ? String(row.delivery_city_id) : "",
...
export function DataProvider({ children }) {
    ...
    const [trackingEvents, setTrackingEvents] = useState(initialTrackingEvents);
    const [hubs, setHubs] = useState([]);
    const [vehicles, setVehicles] = useState([]);
    const [auditLogs, setAuditLogs] = useState(initialAuditLogs);
    const [citiesList, setCitiesList] = useState(cities); // Start with mock data
            ...
            try {
                ...
                if (!cancelled && res && Array.isArray(res.vehicles)) {
                    const mapped = res.vehicles.map((v) => ({
                        ...
                        status: (v.status === 'active' ? 'active' : v.status === 'maintenance' ? 'maintenance' : 'inactive'),
                    }));
                    setVehicles(mapped);
                }
            }
    ...
    const updateParcel = async (id, updates) => {
        try {
            ...
            if (res?.parcel) {
                ...
                setParcels((prev) => prev.map((p) => (p.id === id ? mapApiParcel(res.parcel) : p)));
            }
            else {
    ...
    const updateHub = async (id, updates) => {
        try {
            const apiAny = api;
