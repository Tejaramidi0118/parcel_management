import { Circle, Package, MapPin, Truck, Home } from 'lucide-react';
...
export default function TrackingTimeline({ events }) {
    ...
    const uniqueStatuses = Array.from(new Set(sortedEvents.map(e => e.type))).sort((a, b) => statusOrder.indexOf(a) - statusOrder.indexOf(b));
      ...
      <CardContent>
        <div className="space-y-4">
          {timelineStatuses.map((status, index) => {
            ...
            return (<div key={status} className="relative flex gap-4">
                {/* Timeline line */}
                {index < timelineStatuses.length - 1 && (<div className={`absolute left-5 top-10 w-0.5 h-full ${isCompleted ? 'bg-green-500' : 'bg-gray-300'}`} style={{ height: 'calc(100% + 1rem)' }}/>)}

                {/* Icon */}
                <div className={`relative z-10 flex items-center justify-center w-10 h-10 rounded-full ${isCompleted
                ...
                <div className="flex-1 pb-8">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className={`font-semibold ${isCompleted ? 'text-green-600' : isCurrent ? 'text-primary' : 'text-gray-400'}`}>
                        {config.label}
                      </p>
                      {event && (<>
                          {event.location && (<p className="text-sm text-muted-foreground mt-1">
                              {event.location}
                            </p>)}
                          {event.notes && (<p className="text-sm text-muted-foreground mt-1">
                              {event.notes}
                            </p>)}
                        </>)}
                    </div>
                    {date && (<div className="text-right text-sm text-muted-foreground">
