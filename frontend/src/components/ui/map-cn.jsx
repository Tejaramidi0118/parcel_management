 ...
 */
const Map = ({ ak, option, className, children, ...props }) => {
    ...
    const initMap = useCallback(() => {
        ...
        const center = new window.BMapGL.Point(_options?.lng, _options?.lat);
    ...
    useEffect(() => {
        ...
        if (window.BMapGL) {
            ...
            BMapGLLoadingPromise = loadScript(`//api.map.baidu.com/api?type=webgl&v=1.0&ak=${ak}&callback={{callback}}`);
            BMapGLLoadingPromise.then(initMap).then(() => {
                BMapGLLoadingPromise = null;
            });
        }
