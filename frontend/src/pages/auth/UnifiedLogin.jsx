...
export default function UnifiedLogin() {
  ...
  const determineRoleFromEmail = (emailVal) => {
    if (!emailVal)
      return null;
    const e = emailVal.toLowerCase();
    if (e.endsWith("@swiftadmin.com"))
      return "admin";
    if (e.endsWith("@swiftcourier.com"))
      return "courier";
    if (e.endsWith("@swiftstore.com") || e.includes("store"))
      return "store_owner";
    if (e.includes("@"))
      return "customer";
    return null;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result && result.success) {
        ...
        const roleFromBackend = result.user?.role;
      ...
      <CardContent>
        ...
        <div className="mt-6 space-y-4">
          ...
          <div className="space-y-3">
            {demoAccounts.map((account, index) => (<div key={index} className="p-3 bg-muted/50 rounded-lg border border-border hover:bg-muted transition-colors">
